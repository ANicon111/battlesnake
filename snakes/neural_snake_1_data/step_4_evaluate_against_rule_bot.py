...
from snakes.neural_snake_1 import NeuralAgent, create_app
...
def game_result(game, seed, starts, ends):
    ...
    return {
        ...
        "Longer survival": (
            "Neural"
            if neural_steps > rule_steps
            else "Rule-based" if rule_steps > neural_steps else "Equal"
        ),
        "Points agent 1": ends["Neural"]["you"]["length"]
        - starts["Neural"]["you"]["length"],
        "Points agent 2": ends["Rule-based"]["you"]["length"]
        - starts["Rule-based"]["you"]["length"],
...
def evaluate(
    model="models/rb_trained_nn_model.pt",
    games=10,
    engine="battlesnake/battlesnake.exe",
    seed_start=1000,
    seconds=120,
):
    ...
    apps = [
        scored_app("Neural", neural.move),
        scored_app(
            "Rule-based",
            rule_based_agent.move,
            start=rule_based_agent.start,
            end=rule_based_agent.end,
            info=rule_based_agent.info,
        ),
    ]
    ...
    try:
        ...
        for game in range(games):
            ...
            for name, server in zip(("Neural", "Rule-based"), servers):
                random.seed(seed)
                print(
                    f"Pair {game + 1}/{games}, seed {seed}: {name} solo game",
                    flush=True,
                )
                command = [
                    str(engine),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Seed pairs; each agent plays one solo game per seed",
    )
