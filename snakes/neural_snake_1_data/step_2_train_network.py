...
from neural_snake_1_data.step_0_state_attributes import (
    ACTIONS,
    INPUT_COLUMNS,
    load_dataframe,
)
...
def split_dataframe(df, seed=42):
    ...
    validation = df[group_column].isin(groups[: max(1, round(len(groups) * 0.2))])
    return df.loc[~validation], df.loc[validation]


def train(df, output, epochs=150, learning_rate=0.003):
    output = Path(output)
    if output.exists():
    ...
    y_train = torch.tensor(
        training.direction.map(ACTIONS.index).to_numpy(), dtype=torch.long
    )
    x_validation = torch.tensor(validation[INPUT_COLUMNS].to_numpy(dtype=np.float32))
    y_validation = torch.tensor(
        validation.direction.map(ACTIONS.index).to_numpy(), dtype=torch.long
    )
    ...
    with torch.no_grad():
        accuracy = (
            (model(x_validation).argmax(dim=1) == y_validation).float().mean().item()
        )
    ...
    output.with_suffix(".json").write_text(
        json.dumps({"recorded_seeds": seeds}), encoding="utf8"
    )
