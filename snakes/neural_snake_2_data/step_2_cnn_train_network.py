...
def cnn_split_dataframe(df, seed=42):
    ...
    held_out = df[group_column].isin(groups[: max(1, round(len(groups) * 0.2))])
    return df.loc[~held_out], df.loc[held_out]


def cnn_train(df, output=None, epochs=30, learning_rate=0.003, batch_size=64):
    ...
    print(
        f"Train: {len(training)} decisions; test: {len(test)}. Input: {tuple(x_train.shape)}"
    )
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch == 1 or epoch % 5 == 0 or epoch == epochs:
            print(
                f'Epoch {epoch}: loss {history[-1]["training_loss"]:.3f}; test agreement {accuracy:.1%}'
            )
